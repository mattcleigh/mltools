...
class EmbeddingLayer(nn.Module):
    ...
    def __init__(self, inpt_dim: list, use_context: bool, net_kwargs: dict) -> None:
        """
        args:
            inpt_dim: The dimensions of the input graph [e,n,g,c]
            use_context: Use the global and conditional information in the mlp
        ...
        self.outp_dim = inpt_dim.copy()  ## prevents overwriting
        self.use_context = use_context

        ## The node embedding mlp (default output is the same as input)
        self.dense_net = DenseNetwork(
            inpt_dim=inpt_dim[1],
            ctxt_dim=sum(self.inpt_dim[2:]) if use_context else 0,
            **net_kwargs,
        )
        self.outp_dim[1] = self.dense_net.outp_dim

    def forward(self, graph: GraphBatch) -> GraphBatch:
        """Forward pass for EmbeddingLayer"""

        ## Update the nods of the graph
        graph.nodes = pass_with_mask(
            graph.nodes,
            self.dense_net,
            graph.mask,
            [graph.globs, graph.cndts] if self.use_context else None,
        )
...
class EdgeBuilder(nn.Module):
    ...
    def __repr__(self):
        printout = f"EdgeBuilder({self.clus_type}-{self.clus_val}, n_crd={self.n_crd}"
        printout += f", measure = {self.measure}"
        if self.strip:
            printout += ", strip"
        if self.edge_type != "same":
            printout += f", new_edges={self.edge_type}"
        printout += ")"
        return printout
...
class GraphNetwork(nn.Module):
    """A series of consecutive Graph-Network blocks and Edge-Building layers
    - Edge Building is always the first step in this process
    """
    ...
    def __init__(
        ...
        args:
            inpt_dim: The dimensions of the input graph [e,n,g,c]
        kwargs:
            depth: The number of new graph stages
            start_with_ebl: If the network begins with an edge building layer
            gnb_kwargs: A dictionary holding all args for GNBlock() except inpt dimension
        ...
        if isinstance(ebl_kwargs, list):
            if len(ebl_kwargs) != depth - 1 + start_with_ebl:
                raise ValueError("len of EdgeBuilder kwargs does not match depth")
            ebl_list = ebl_kwargs.copy()
            for i in range(1, depth):
                ebl_list[i] = merge_dict(ebl_list[0], ebl_list[i])
        else:
            ebl_list = depth * [ebl_kwargs]

        if isinstance(gnb_kwargs, list):
            if len(gnb_kwargs) != depth:
                raise ValueError("len of GNBlock kwargs does not match network depth")
            gnb_list = gnb_kwargs.copy()
            for i in range(1, depth):
                gnb_list[i] = merge_dict(gnb_list[0], gnb_list[i])
        else:
            gnb_list = depth * [gnb_kwargs]

        ## Store the input dimensions of the entire network
        self.inpt_dim = inpt_dim
...
class GraphGenerator(nn.Module):
    """A simple graph based point cloud generator which takes in a latent space input.

    The point cloud is generated in four stages.
    ...
    4. Embeds the nodes back into the desired space to match the output dimension.

    The mask of the output graph must be known beforehand.
    """
    ...
    def __init__(
        ...
        self.gnn = GraphNetwork(
            [0, self.node_init_dim, self.dns.outp_dim, cndt_dim], **gnn_kwargs
        )

        ## The output embedding layer for the nodes
        emb_kwargs["net_kwargs"]["outp_dim"] = outp_dim[1]
        self.emb = EmbeddingLayer(self.gnn.outp_dim, **emb_kwargs)

        ## Set the final output dimension of this module
        self.outp_dim = self.emb.outp_dim

    def _create_init_graph(
        self, inputs: T.Tensor, target_mask: T.BoolTensor, ctxt: T.Tensor
    ) -> GraphBatch:
        """Returns the initial random graph object converted from the vector
        with the correct attributes from point 2. above
        """
    ...
    def forward(
        ...
        ## Pass the input through the dense network
        inputs = self.dns.forward(inputs, ctxt=ctxt)

        ## Create the random graph with correct masking and inputs as globals
        inputs = self._create_init_graph(inputs, target_mask, ctxt)

        ## Pass the random graph through the graph network to update
        inputs = self.gnn(inputs)

        ## Pass through the embedding layer to ensure proper output size
